        ...
        for k,v in self.iteritems():
            if k in obj.list_vars():
                if isinstance(getattr(self, k), dictree):
                    v = getattr(self, k).VariableTree(getattr(obj,k).__class__)
                setattr(obj, k, v)
        return obj
...
def list_ios(self, iotype=None):
    """ Only list the I/Os that are not in the bease Assembly class"""
    if not iotype:
...
def my_tree(self, iotype=None):
    if hasattr(self, 'list_containers'):
        containers = [f for f in self.list_containers() if f not in black_list and any([isinstance(getattr(self,f), o) for o in (Component, Driver, Assembly, VariableTree)])]
...
def my_str(var, parent='', value=False):
    ...
    if isinstance(var, VariableTree):
        out = '{' + '<'+ parent + '> ' +  parent + '|{' + '|'.join([my_str(v,k, value) for k,v in var.items()]) + '}}'
    else:
        if value: 
...
def out_graph_component(self, graph, value=False):
    ...
    * value: print the value (default False)
    ...
    graph.add_node(name, label='%s|{{%s}|{%s}}'%(name,
            '|'.join([my_str(getattr(self,n),n,value) for n in list_ios(self, 'in')]),
...
def out_graph_driver(self, graph, value=False):
    ...
    * value: print the value (default False)
    """
    c1 = graph.add_subgraph(name='cluster_'+self.name, label=self.name, color='orange', style='rounded')
    c1.add_node(self.name+'_in', label='Inputs|%s'%('|'.join([my_str(getattr(self,n),n,value) for n in list_ios(self, 'in')])), style='rounded', shape = "record")
    c1.add_node(self.name+'_out', label='Outputs|%s'%('|'.join([my_str(getattr(self,n),n,value) for n in list_ios(self, 'out')])), style='rounded', shape = "record")
...
def out_graph_assembly(self, graph, value=False):
    ...
    * value: print the value (default False)
    ...
    for e in self._depgraph.edges():
        if len(e[0].split('.'))>2 or len(e[1].split('.'))>2: continue
        ...
        if '.' not in e[0] and '.' in e[1]:            
            ...
            if all([e_ not in Assembly.__base_traits__ for e_ in [e[0], e10, e11]]):
                if e[0] in self.list_ios('in'):
                    graph.add_edge(self.name+'_in', e10, tailport=e[0], headport=e11)
        if '.' in e[0] and '.' not in e[1]:
            e00, e01 = e[0].split('.')
            if all([e_ not in Assembly.__base_traits__ for e_ in [e00, e01, e[1]]]):
                if e[1] in self.list_ios('out'):
                    graph.add_edge(e00, self.name+'_out', tailport=e01, headport=e[1])
...
def interactive_graph(self):
    ...
    inputs = dict([(c,(getattr(basedict[c],'low'), getattr(basedict[c],'high'))) for c in list_ios(self, 'in')])
    def f(**kwargs): 
        my_call(self, **kwargs)
...
class pandasMDAO(object):
    ...
    def run(self, a, run_callback='run'):
        if self.exists(a, iotype='in'):
            print 'returning'
            state = {k: v for k,v in zip(self.df.keys(), self.df[self.bool_arr(a, iotype='in').all(0)].values[0])}
            ...
            self.append(a)
            #return {k: v for k,v in zip(self.df.keys(), self.df[self.bool_arr(a, iotype='in').all(0)].values[0])}
...
def lconnect(self, a, b):
    ...
    for v in set_a.intersection(set_b):
        try: 
            ...
            ## No crash when the connection is failing, just a warning printing the error
